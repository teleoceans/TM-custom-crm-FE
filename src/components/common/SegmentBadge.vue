<template>
  <component
    ...
    :class="[
      ...
      { 'segment-badge--interactive': interactive },
    ]"
    :data-entity="entityType"
    :aria-label="ariaLabel || label"
  >
    <span class="segment-badge__icon" aria-hidden="true">
      <svg
        v-if="variant === 'all'"
        width="12"
        height="12"
    ...
    <span class="segment-badge__label">
      <slot>{{ label }}</slot>
    </span>
  </component>
</template>
...
const props = defineProps({
  entityType: {
    type: String,
    required: true,
    validator: (value) => ["user", "tenant"].includes(value),
  },
  variant: {
    type: String,
    default: "all",
...
.segment-badge--interactive {
  cursor: pointer;
}

/* Dark mode */
:global(.dark) .segment-badge--all {
  background: #cabffd;
  border-color: #5521b5;
  color: #5521b5;
}

:global(.dark) .segment-badge--new {
  background: #def7ec;
  border-color: #e5e7eb;
  color: #28a745;
}
